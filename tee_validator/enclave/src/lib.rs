...
#[no_mangle]
pub unsafe extern "C" fn ecall_get_ecc_signing_pubkey(
    pubkey: *mut u8,
    pubkey_size: usize,
) -> sgx_status_t {
    validate_const_ptr(pubkey, pubkey_size as usize);

    if !ed25519::does_key_exist() {
        info!("Key not set. Use --generate or --import");
...
#[no_mangle]
pub unsafe extern "C" fn ecall_generate_key(password: *const u8, pass_size: usize) -> sgx_status_t {
    ...
    //
    if let Err(status) = ed25519::create_sealed_seed(&aes_key) {
        return status;
    }
...
#[no_mangle]
pub unsafe extern "C" fn ecall_import_key(
    key: *const u8,
    key_size: usize,
    password: *const u8,
    pass_size: usize,
...
#[no_mangle]
pub unsafe extern "C" fn ecall_export_key(
    password: *const u8,
    pass_size: usize,
    key: &mut [u8; 32],
) -> sgx_status_t {
